...
import jakarta.servlet.http.HttpSession;  // Spring Boot 3.x 버전
...
class SessionInfo {
	...
	public SessionInfo(HttpSession session) {
		...
		this.creationTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
			.format(new Date(session.getCreationTime()));
	}
}