...
public class CashController {
	...
	@Operation(summary = "현금 잔액 조회", description = "사용자의 현금 잔액을 조회합니다.", security = @SecurityRequirement(name = "bearer-token"))
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "현금 잔액 조회 성공", content = @Content(schema = @Schema(implementation = String.class), examples = @ExampleObject(value = "{\"success\": true, \"message\": \"현금 잔액 조회 성공\", \"data\": {\"cash\": 500000}}"))),
	...
	public ResponseEntity<Map<String, Object>> getCashBalance() {
		// 예제 데이터 (실제로는 DB에서 조회)
		Map<String, Object> response = Map.of("success", true, "message", "현금 잔액 조회 성공", "data",
				Map.of("cash", 500000));
