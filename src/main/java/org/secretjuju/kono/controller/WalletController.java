...
public class WalletController {
	...
	@ApiResponses(value = {
		@ApiResponse(
			responseCode = "200",
			description = "보유 코인 목록 조회 성공",
			content = @Content(
				schema = @Schema(implementation = String.class),
				examples = @ExampleObject(value = "{\"success\": true, \"message\": \"보유 코인 목록 조회 성공\", \"data\": []}")
			)
		),
		@ApiResponse(
			responseCode = "401",
	...
	@ApiResponses(value = {
		@ApiResponse(
			responseCode = "200",
			description = "특정 코인 보유 정보 조회 성공",
			content = @Content(
				schema = @Schema(implementation = String.class),
				examples = @ExampleObject(value = "{\"success\": true, \"message\": \"특정 코인 보유 정보 조회 성공\", \"data\": {\"hasHolding\": true}}")
			)
		),
		@ApiResponse(
			responseCode = "401",
